    ...
    def word2vec(self, word1, word2):
        ...
        if self.punctuation is False:
            pun_list = ['。', '，', '、', '？', '！', '；', '：', '“', '”', '‘', '’', '「', '」', '『', '』', '（', '）', '[', ']',
                        '〔', '〕', '【', '】', '——', '—', '……', '…', '—', '-', '～', '·', '《', '》', '〈', '〉', '﹏﹏', '___',
                        '.']
            seg_list_1 = [w for w in word1.split(' ') if w not in pun_list]
            seg_list_2 = [w for w in word2.split(' ') if w not in pun_list]
        else:
            seg_list_1 = word1.split(' ')
            seg_list_2 = word2.split(' ')
